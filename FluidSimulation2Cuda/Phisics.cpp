...
double Math::calculateSlope(Vector2D a, Vector2D b) {
...
Vector2D Math::calculateNormalVector(double surfaceLineSlope) {
...
Vector2D Math::calculateReflectionVector(const Vector2D& incidentVector, const Vector2D& normalVector) {
...
float Math::squared_distance(const Vector2D& p1, const Vector2D& p2) {
	float dx = p1.X - p2.X;
	float dy = p1.Y - p2.Y;
	return dx * dx + dy * dy;
}
...
bool Math::check_line_segment_circle_intersection(const Vector2D& A, const Vector2D& B, const Vector2D& C, float radius) {
...
float Math::smoothingKernel(float radius, float distance) {
	if (distance >= radius) {
		return 0.0f;
	}
...
float Math::smoothingKernelDerivative(float radius, float distance) {
	if (distance >= radius) {
		return 0.0f;
	}
	float x = (radius - distance) / radius;
...
float Math::viscositySmoothingKernel(float radius, float distance) {
	if (distance >= radius) {
		return 0.0f;
	}
