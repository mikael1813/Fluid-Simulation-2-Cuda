...
__device__ void updateParticle(int index, Particle* particle, float gravity, double dt) {
	if (dt == 0) {
		return;
	}
...
__device__ GpuVector2D calculatePressureForce(int index, Particle* particles, int praticlesSize, int particleRadiusOfRepel,
	...
	float targetDensity, float pressureCoeficient)
{
	...
	for (int i = -1; i < 2; i++) {
		for (int j = -1; j < 2; j++) {
			...
			for (int otherParticleIndex = lengths[lengthIndex].start; otherParticleIndex < lengths[lengthIndex].end; otherParticleIndex++) {
				...
				float sharedPressure = CudaMath::calculateSharedPressure(particle.m_Density, otherParticle.m_Density, targetDensity, pressureCoeficient);
...
__device__ GpuVector2D calculateViscosityForce(int index, Particle* particles, int praticlesSize, int particleRadiusOfRepel,
	int particleRadius, Range* lengths, int interactionMatrixRows, int interactionMatrixCols, float viscosityStrength) {
...
__device__ GpuVector2D calculateSurfaceTensionForce(int index, Particle* particles, int praticlesSize, int particleRadiusOfRepel,
	int particleRadius, Range* lengths, int interactionMatrixRows, int interactionMatrixCols, float averageDensity) {
...
__device__ void updateParticleFutureVelocities(int index, Particle* particles, int praticlesSize,
	...
	size_t interactionMatrixCols, double dt, float averageDensity, bool applySurfaceTension, float viscosityStrength,
	float targetDensity, float pressureCoeficient, float gravity)
{
	...
	GpuVector2D pressureForce = calculatePressureForce(index, particles, praticlesSize, particleRadiusOfRepel, particleRadius,
		lengths, interactionMatrixRows, interactionMatrixCols, targetDensity, pressureCoeficient);
	...
	GpuVector2D viscosityForce = calculateViscosityForce(index, particles, praticlesSize, particleRadiusOfRepel, particleRadius,
		lengths, interactionMatrixRows, interactionMatrixCols, viscosityStrength);
	//GpuVector2D viscosityForce = GpuVector2D();
	...
	updateParticle(index, particles, gravity, dt);
...
__global__ void specialUpdateFutureVelocities(Particle* particles, int praticlesSize, int particleRadiusOfRepel,
	...
	float viscosityStrength, float targetDensity, float pressureCoeficient, float gravity) {
	...
	updateParticleFutureVelocities(index, particles, praticlesSize, particleRadiusOfRepel,
		...
		applySurfaceTension, viscosityStrength, targetDensity, pressureCoeficient, gravity);
}
...
void GpuAllocate(std::vector<Particle>& particles, std::vector<Surface2D>& obstacles, int interactionMatrixSize, std::vector<ConsumerPipe>& consumerPipes,
	...
	cudaStatus = cudaMalloc(&deviceParticlePressureObstacles, particles.size() * sizeof(float) * 4);
...
void UpdateParticlesHelper(std::vector<Particle>& particles, int particlesSize, int particleRadiusOfRepel,
	...
	size_t interactionMatrixCols, float averageDensity, bool applySurfaceTension, float viscosityStrength,
	float targetDensity, float pressureCoeficient, float gravity) {
	...
	specialUpdateFutureVelocities << <numBlocks, blockSize >> > (deviceParticles, particlesSize, particleRadiusOfRepel,
		...
		targetDensity, pressureCoeficient, gravity);
...
__global__ void GpuGetWallPressure(Particle* particles, int particlesSize, int screenWidth, int screenHeight,
	int particleRadius, double dt, float* deviceParticlePressureObstacles) {
	...
	deviceParticlePressureObstacles[particleIndex + obstacleIndex * particlesSize] = 0.0f;

	switch (obstacleIndex) {
	case 3:
		// Left wall
		if (particles[particleIndex].m_Position.X < particleRadius * 2) {
			deviceParticlePressureObstacles[particleIndex + obstacleIndex * particlesSize] =
				particles[particleIndex].m_Velocity.X < 0 ?
				particles[particleIndex].m_Velocity.X * particles[particleIndex].m_Mass / dt :
				0.0f;
		}
		break;
	case 1:
		// Right wall
		if (particles[particleIndex].m_Position.X > screenWidth - particleRadius * 2) {
			deviceParticlePressureObstacles[particleIndex + obstacleIndex * particlesSize] =
				particles[particleIndex].m_Velocity.X > 0 ?
				particles[particleIndex].m_Velocity.X * particles[particleIndex].m_Mass / dt :
				0.0f;
	...
	case 0:
		// Top wall
		if (particles[particleIndex].m_Position.Y < particleRadius * 2) {
			deviceParticlePressureObstacles[particleIndex + obstacleIndex * particlesSize] =
				particles[particleIndex].m_Velocity.Y < 0 ?
				particles[particleIndex].m_Velocity.Y * particles[particleIndex].m_Mass / dt :
				0.0f;
		}
		break;
	case 2:
		// Bottom wall
		if (particles[particleIndex].m_Position.Y > screenHeight - particleRadius * 2) {
			deviceParticlePressureObstacles[particleIndex + obstacleIndex * particlesSize] =
				particles[particleIndex].m_Velocity.Y > 0 ?
				particles[particleIndex].m_Velocity.Y * particles[particleIndex].m_Mass / dt :
				0.0f;
...
void GpuUpdateParticles(std::vector<Particle>& particles, int& particlesSize, int particleRadiusOfRepel,
	...
	bool applySurfaceTension, float viscosityStrength, float targetDensity, float pressureCoeficient,
	float gravity, std::vector<float>& wallPressure) {
	...
	UpdateParticlesHelper(particles, particlesSize, particleRadiusOfRepel, particleRadius, particleRepulsionForce,
		...
		viscosityStrength, targetDensity, pressureCoeficient, gravity);
	...
	GpuGetWallPressure << <dim3(particles.size(), 4), maxThreadsPerBlock >> > (deviceParticles, particles.size(), 1800, 900,
		particleRadius, dt, deviceParticlePressureObstacles);

	cudaDeviceSynchronize();

	float* outputPressureObstacles = new float[particles.size() * 4];

	cudaMemcpy(outputPressureObstacles, deviceParticlePressureObstacles, particles.size() * sizeof(float) * 4, cudaMemcpyDeviceToHost);

	std::vector<float> pressureObstacles;

	for (int j = 0; j < 4; j++) {
		pressureObstacles.push_back(0.0f);
		for (int i = 0; i < particles.size(); i++) {
			pressureObstacles[j] += outputPressureObstacles[i + j * particles.size()];
		}
		wallPressure[j] = pressureObstacles[j];
	}
